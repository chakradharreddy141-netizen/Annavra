  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4 pb-24 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold text-[#1a1a1a] tracking-tight font-space">Workout Hub</h1>
      </div>

      {/* Today's Plan */}
      <div className="cyber-panel rounded-2xl overflow-hidden">
        <div className={`p-6 ${isRestDay ? 'bg-[#ff4500]/10' : 'bg-[#1a1a1a]/5'} border-b ${isRestDay ? 'border-[#1a1a1a]/10' : 'border-[#1a1a1a]/10'} relative overflow-hidden`}>
          <div className="absolute -right-4 -top-4 opacity-10">
            {isRestDay ? <Calendar className="w-32 h-32" /> : <Dumbbell className="w-32 h-32" />}
          </div>
          
          <span className={`text-xs font-bold uppercase tracking-wider ${isRestDay ? 'text-[#ff4500]' : 'text-[#1a1a1a]'}`}>
            Today's Plan
          </span>
          <h2 className="text-3xl font-black text-[#1a1a1a] mt-1 relative z-10 font-space">
            {isRestDay ? 'Rest Day' : todayWorkoutName}
          </h2>
          
          {alreadyWorkedOutToday && (
            <div className="flex items-center gap-1.5 mt-3 text-sm font-semibold text-[#ff4500]">
        ...
        <div className="p-6 bg-[#f7f7f5]/50">
          {isRestDay ? (
          ...
          <Link
            href="/workout/active"
            className="w-full py-4 flex items-center justify-center gap-2 btn-cyber rounded-xl font-bold transition-all active:scale-95 text-[#1a1a1a]"
          >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <h2 className="text-lg font-bold text-[#1a1a1a] font-space">Recent Workouts</h2>
        </div>

        {!recentWorkouts || recentWorkouts.length === 0 ? (
          <div className="py-10 text-center border border-dashed border-[#1a1a1a]/10 rounded-2xl cyber-panel">
            <div className="w-12 h-12 rounded-full bg-[#1a1a1a]/5 flex items-center justify-center mx-auto mb-3">
              <Clock className="w-5 h-5 text-[#1a1a1a] glow-purple" />
            </div>
            <p className="text-sm font-medium text-[#1a1a1a]">No recent workouts</p>
            <p className="text-xs text-[#6b7280] mt-1">Your logged sessions will appear here.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {recentWorkouts.map((workout) => (
              <div key={workout.id} className="p-4 rounded-xl cyber-panel cyber-panel-hover flex items-center justify-between group">
                <div>
                  <div className="font-semibold text-sm text-[#1a1a1a] font-space flex items-center gap-2">
                    {workout.name || workout.workout_type}
                    {workout.workout_sets?.some((s: any) => s.is_pr) && (
                      <Trophy className="w-4 h-4 text-[#1a1a1a]" style={{ filter: 'drop-shadow(0 0 4px #a855f7)' }} />
                    )}
                  </div>
